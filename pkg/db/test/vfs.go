...
func VfsFile(t *testing.T, dbo orm.DB, in *db.VfsFile, ops ...VfsFileOpFunc) (*db.VfsFile, Cleaner) {
	...
	if in.ID != 0 {
		...
		t.Logf("the entity VfsFile is not found by provided PKs, ID=%v. Trying to create one", in.ID)
	}
...
func WithFakeVfsFile(t *testing.T, dbo orm.DB, in *db.VfsFile) Cleaner {
	...
	if in.Path == "" {
		in.Path = cutS(gofakeit.Sentence(), 255)
	}

	if in.MimeType == "" {
		in.MimeType = cutS(gofakeit.Sentence(), 255)
	}
...
func VfsFolder(t *testing.T, dbo orm.DB, in *db.VfsFolder, ops ...VfsFolderOpFunc) (*db.VfsFolder, Cleaner) {
	...
	if in.ID != 0 {
		...
		// If we're here, we don't find the entity by PKs. Just try to add the entity by provided PK
		t.Logf("the entity VfsFolder is not found by provided PKs, ID=%v. Trying to create one", in.ID)
	}
